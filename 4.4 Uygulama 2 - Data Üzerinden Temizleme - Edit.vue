...
export default{
    ...
    mounted(){
        ...
        const url = 'http://localhost:3000/Projeler/' + id

        fetch(url)
        .then((res)=>res.json())
        .then((data)=>{
            //Bunları yerleştireceğiz.
            this.title = data.isim;
            this.details = data.bilgi
        })
    ...
    methods:{
        EditScript(){
            ...
            fetch(url,{
                ...
                body: JSON.stringify({isim: this.title, bilgi: this.details})
            })
